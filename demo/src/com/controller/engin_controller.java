...
import com.entity.engin;

@zw_web
public class engin_controller {
	zw_json json=new zw_json();
	
	
	@zw_path("/engin_synchronous")
...
//			
	...
	@zw_path("/engin_query_title")
	public String engin_query_title(HttpServletRequest request,HttpServletResponse response){
		service vo=new service();
		
		jdbc_return return_jdbc=factory_class.jdbc_execute_sql(vo, static_class.jdbc_type_select,"select id ,title  from engin where parentId=0");
		web_return return_object=new web_return("查询成功",return_jdbc.getList(),return_jdbc.getCount());
		return  json.toJsonString(return_object);
	}
