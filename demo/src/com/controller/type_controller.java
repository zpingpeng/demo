...
//			
	...
	@zw_path("/type_query_title")
	public String type_query_title(HttpServletRequest request,HttpServletResponse response){
		service vo=new service();
		
		jdbc_return return_jdbc=factory_class.jdbc_execute_sql(vo, static_class.jdbc_type_select,"select id ,title  from type where parentId=0");
		web_return return_object=new web_return("查询成功",return_jdbc.getList(),return_jdbc.getCount());
		return  json.toJsonString(return_object);
	}
